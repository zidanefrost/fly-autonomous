"""~100 popular airports worldwide, excluding the USA and Russia, keyed by ICAO code.

Each record: (icao, iata, name, city, country, lat, lon).
Used to seed Supabase and as the fetch list for the METAR worker.
"""

AIRPORTS: list[tuple[str, str, str, str, str, float, float]] = [
    # --- Europe ---
    ...
    ("EDDM", "MUC", "Munich", "Munich", "Germany", 48.3537, 11.7860),
    ...
    ("EKCH", "CPH", "Copenhagen", "Copenhagen", "Denmark", 55.6180, 12.6560),
    ...
    ("LKPR", "PRG", "Prague", "Prague", "Czech Republic", 50.1008, 14.2600),
    ...
    ("OMAA", "AUH", "Abu Dhabi", "Abu Dhabi", "United Arab Emirates", 24.4330, 54.6511),
    ("OTHH", "DOH", "Doha Hamad", "Doha", "Qatar", 25.2731, 51.6080),
    ...
    # --- Asia ---
    ("VHHH", "HKG", "Hong Kong Intl", "Hong Kong", "Hong Kong", 22.3080, 113.9185),
    ...
    ("RJBB", "KIX", "Osaka Kansai", "Osaka", "Japan", 34.4347, 135.2440),
    ...
    ("VTBS", "BKK", "Bangkok Suvarnabhumi", "Bangkok", "Thailand", 13.6900, 100.7501),
    ...
    ("VVTS", "SGN", "Ho Chi Minh City", "Ho Chi Minh City", "Vietnam", 10.8188, 106.6520),
    ("VIDP", "DEL", "Delhi Indira Gandhi", "Delhi", "India", 28.5562, 77.1000),
    ...
    ("YMML", "MEL", "Melbourne", "Melbourne", "Australia", -37.6690, 144.8410),
    ("YBBN", "BNE", "Brisbane", "Brisbane", "Australia", -27.3942, 153.1218),
    ("YPPH", "PER", "Perth", "Perth", "Australia", -31.9403, 115.9669),
    ("NZAA", "AKL", "Auckland", "Auckland", "New Zealand", -37.0082, 174.7850),
    # --- Africa ---
    ("HECA", "CAI", "Cairo Intl", "Cairo", "Egypt", 30.1219, 31.4056),
    ("FACT", "CPT", "Cape Town", "Cape Town", "South Africa", -33.9649, 18.6017),
    ("FAOR", "JNB", "Johannesburg O.R. Tambo", "Johannesburg", "South Africa", -26.1392, 28.2460),
    ("GMMN", "CMN", "Casablanca Mohammed V", "Casablanca", "Morocco", 33.3675, -7.5900),
    ...
    ("DAAG", "ALG", "Algiers Houari Boumediene", "Algiers", "Algeria", 36.6910, 3.2154),
    ("DTTA", "TUN", "Tunis Carthage", "Tunis", "Tunisia", 36.8510, 10.2272),
    ...
    ("SBGL", "GIG", "Rio de Janeiro Galeão", "Rio de Janeiro", "Brazil", -22.8090, -43.2436),
    ("SAEZ", "EZE", "Buenos Aires Ezeiza", "Buenos Aires", "Argentina", -34.8222, -58.5358),
    ("SCEL", "SCL", "Santiago", "Santiago", "Chile", -33.3930, -70.7858),
...
ICAO_CODES: list[str] = [row[0] for row in AIRPORTS]

EXCLUDED_COUNTRIES = {"United States", "USA", "Russia"}

assert len(ICAO_CODES) == len(set(ICAO_CODES)), "duplicate ICAO code in seed list"
assert 90 <= len(AIRPORTS) <= 110, f"expected ~100 airports, got {len(AIRPORTS)}"
