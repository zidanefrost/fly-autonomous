...
One `modal.App` with several functions, deployed together via `modal deploy worker/app.py`:
  1. `refresh_all_airports` — cron, every 10 min: fetch -> score -> upsert.
...
def _route_forecast(
    ...
    """Given already-fetched TAFs, compute the worst-of departure/arrival
    forecast risk for `target_epoch`."""
...
def refresh_all_airports():
    ...
    print(f"Refreshed {result['written']} airports; missing={result['missing']}")
